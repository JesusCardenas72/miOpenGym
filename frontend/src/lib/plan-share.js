...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR HipertroFit — just the
...
export function parsePlan(raw) {
  ...
  if (!data || !data.opengym_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t an HipertroFit plan file'))
  }
...
<body><div class="doc">
  <header>
    <div class="kicker">HipertroFit</div>
  ...
  ${body}
  <footer>${esc(t('Made with HipertroFit'))} · opengym.duarte-santos.ch</footer>
</div></body></html>`
}
