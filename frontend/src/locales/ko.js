...
export default {
  ...
  "This browser doesn't support passkeys — you can still use HipertroFit locally on this device.": '이 브라우저는 패스키를 지원하지 않아요 — 그래도 이 기기에서 HipertroFit을 로컬로 사용할 수 있습니다.',
  ...
  'to install HipertroFit as a full-screen app.': '하면 HipertroFit을 전체 화면 앱으로 설치할 수 있어요.',
  ...
  'Rest-timer alerts, even if HipertroFit is closed.': 'HipertroFit이 닫혀 있어도 휴식 타이머 알림을 받아요.',
  ...
  'A small file a friend imports into their own HipertroFit — routines only, none of your workouts or weigh-ins.': '친구가 자신의 HipertroFit으로 가져올 수 있는 작은 파일 — 루틴만 포함되며 운동 기록이나 체중은 포함되지 않습니다.',
  ...
  'Your server': '내 서버',
  'Synced with your HipertroFit server.': 'HipertroFit 서버와 동기화됨',
  'Disconnect': '연결 해제',
  ...
  'Sync this device to your own self-hosted HipertroFit instead.': '대신 이 기기를 직접 호스팅하는 HipertroFit 서버와 동기화합니다.',
  'Pair the mobile app': '모바일 앱 페어링',
  'Connect the HipertroFit app on your phone to this account.': '휴대폰의 HipertroFit 앱을 이 계정에 연결하세요.',
  'Could not generate a code': '코드를 생성하지 못했습니다',
  'On the HipertroFit app, choose “Connect to my server”, then enter this address and the code below. It expires in 5 minutes.': 'HipertroFit 앱에서 "내 서버에 연결"을 선택한 다음, 이 주소와 아래 코드를 입력하세요. 5분 후 만료됩니다.',
  'Enter your server address and the code': '서버 주소와 코드를 입력하세요',
  'Connected': '연결됨',
  'Could not connect': '연결하지 못했습니다',
  'Open Settings → “Pair the mobile app” on the HipertroFit site you’re already signed into, then enter its address and the code shown there.': '이미 로그인된 HipertroFit 사이트에서 설정 → "모바일 앱 페어링"을 열고, 주소와 거기에 표시된 코드를 입력하세요.',
  'Server address (e.g. gym.example.com)': '서버 주소 (예: gym.example.com)',
  'Pairing code': '페어링 코드',
  'Connecting…': '연결 중…',
  'Connect': '연결',
  'How do you want to use HipertroFit?': 'HipertroFit을 어떻게 사용하시겠어요?',
  'Use on this device': '이 기기에서 사용',
  'Local keeps everything on this phone. Connecting syncs to your own HipertroFit server instead — you can switch later in Settings.': '로컬 모드는 모든 것을 이 휴대폰에 보관합니다. 연결하면 대신 자신의 HipertroFit 서버와 동기화됩니다 — 나중에 설정에서 변경할 수 있습니다.',
