...
export function Row({ icon, iconTint, title, subtitle, value, accessory = 'none', onClick, danger, children, className = '' }) {
  ...
  return (
    <Tag className={'lrow' + (onClick ? ' tap' : '') + (danger ? ' danger' : '') + ' ' + className} onClick={onClick}>
      ...
      {/* Controls go in their own box rather than loose in the row, so the stylesheet has one
          thing to reason about however many a row is given, and a row added later adapts
          without needing a rule of its own. See .lrow-ctl. */}
      {children && <span className="lrow-ctl">{children}</span>}
...
export function SelectRow({ icon, iconTint, title, value, options, onChange, sheetTitle, stackedValue = false, search, className = '' }) {
  ...
  return (
    <Row icon={icon} iconTint={iconTint} title={title} value={cur ? cur.label : value} accessory="chevron" onClick={open}
      className={(stackedValue ? 'lrow-stack-value' : '') + ' ' + className} />
  )
}
